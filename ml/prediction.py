...
class BrokeClassification():
    def __init__(self, data: pd.DataFrame, target: pd.DataFrame):
        ...
        X_train, y_train, X_test, y_test = train_test_split(data, target, test_size=0.3, random_state=RANDOM_STATE)
    ...
    def param_selection(self):
        ...
        train_data = Pool(data=self.data, label=self.target, cat_features=self.categorical_features)
